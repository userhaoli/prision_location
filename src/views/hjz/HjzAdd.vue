<template>
  <div class="defence-build">
    ...
    <div class="type">
      ...
      <div class="type-item" v-show="$store.state.defenceActive > 0">
        <span>互监距离</span>
        <input type="text" v-model="$store.state.fenceData.distance" placeholder="互监距离" />
      </div>
      <div class="type-item" v-show="$store.state.defenceActive > 1">
        <span>解除时间</span>
...
export default {
  ...
  watch: {
    ...
    "$store.state.fenceData.distance": {
      handler(val) {
        if (val !== "" && this.$store.state.defenceActive === 1) {
          this.$store.state.defenceActive = 2;
        } else if (val === "" && this.$store.state.defenceActive < 2) {
          this.$store.state.defenceActive = 1;
          this.$message.info("请设置互监距离！");
        } else if (val === "" && this.$store.state.defenceActive > 2) {
          this.$message.info("请设置互监距离！");
        }
      }
    },
    "$store.state.fenceData.over_time": {
      handler(val) {
        if (val !== "" && this.$store.state.defenceActive === 2) {
          this.$store.state.defenceActive = 3;
        } else if (val === "" && this.$store.state.defenceActive < 2) {
          this.$store.state.defenceActive = 2;
          this.$message.info("请设置解除时间！");
        } else if (val === "" && this.$store.state.defenceActive > 2) {
          this.$message.info("请设置解除时间！");
        }
      }
